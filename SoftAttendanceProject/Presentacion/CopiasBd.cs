...
namespace SoftAttendanceProject.Presentacion
{
    public partial class CopiasBd : UserControl
    {
        ...
        string ruta;
        string txtSoftware = "SoftAttendance";
        string Base_De_datos = "SOFT_ATTENDANCE";
        private Thread Hilo;
        ...
        private void ejecucion()
        {
            ...
            try
            {
                string v_nombre_espaldo = Base_De_datos + ".bak";
                CONEXIONMAESTRA.abrir();
                SqlCommand cmd = new SqlCommand("BACKUP DATABASE " + Base_De_datos + " TO DISK = '" + SubCarpeta + @"\" + v_nombre_espaldo + "'", CONEXIONMAESTRA.conectar);
                cmd.ExecuteNonQuery();
                acaba = true;
            }
            catch (Exception ex)
